...
Trong giai đoạn đầu của mạng nơ-ron, các hàm kích hoạt phổ biến chủ yếu được lấy cảm hứng từ sinh học thần kinh, đặc biệt là các hàm có dạng chữ S (sigmoidal). Hai đại diện tiêu biểu là sigmoid và tanh.

Mặc dù ngày nay chúng ít được sử dụng trong các mạng sâu hiện đại, việc hiểu rõ ưu và nhược điểm của chúng cung cấp insight quan trọng về động học tối ưu (optimization dynamics) trong mạng nơ-ron.

\begin{description}
	\item[Sigmoid (Logistic):] Hàm sigmoid ánh xạ một giá trị thực bất kỳ vào khoảng $(0, 1)$:
	\begin{equation}
		\sigma(x) = \frac{1}{1 + e^{-x}}
	\end{equation}
	
	Hàm này từng được ưa chuộng vì có thể diễn giải như xác suất kích hoạt của một nơ-ron. Tuy nhiên, nó tồn tại nhiều vấn đề nghiêm trọng trong huấn luyện mạng sâu.
	
	\paragraph{(1) Vấn đề Suy giảm Gradient (Vanishing Gradient)}
	
	Đạo hàm của sigmoid là:
	\[
	\sigma'(x) = \sigma(x)(1 - \sigma(x))
	\]
	
	Giá trị cực đại của đạo hàm là $0.25$ tại $x=0$, và nhanh chóng tiến về 0 khi $|x|$ lớn.
	
	Khi lan truyền ngược qua nhiều lớp:
	\begin{itemize}
		\item Gradient bị nhân liên tiếp với các giá trị nhỏ hơn 1
		\item Dẫn đến suy giảm theo hàm mũ
	\end{itemize}
	
	Kết quả:
	\begin{itemize}
		\item Các lớp gần input gần như không nhận được gradient
		\item Mạng không thể học các biểu diễn sâu
	\end{itemize}
	
	\paragraph{(2) Output không Zero-centered và Hiệu ứng Zig-zag}
	
	Output của sigmoid luôn dương:
	\[
	\sigma(x) \in (0, 1)
	\]
	
	Điều này dẫn đến một hệ quả tinh vi nhưng rất quan trọng trong tối ưu hóa.
	
	Xét gradient của một trọng số $w$ trong một neuron:
	\[
	\frac{\partial L}{\partial w} = \delta \cdot x
	\]
	
	trong đó $x$ là đầu vào của neuron.
	
	Vì $x > 0$ (do sigmoid):
	\begin{itemize}
		\item Gradient theo $w$ luôn có cùng dấu với $\delta$
	\end{itemize}
	
	Khi cập nhật nhiều trọng số:
	\begin{itemize}
		\item Các thành phần gradient có xu hướng cùng dấu
		\item Vector gradient bị lệch (biased)
	\end{itemize}
	
	\begin{mdframed}[style=infobox]
		\textbf{Hiệu ứng Zig-zag}
		
		Do gradient không cân bằng quanh 0, quá trình cập nhật tham số không đi thẳng theo hướng tối ưu mà bị dao động qua lại (zig-zag), đặc biệt trong không gian tham số nhiều chiều. Điều này làm giảm tốc độ hội tụ đáng kể.
	\end{mdframed}
	
	Trực giác:
	\begin{itemize}
		\item Nếu dữ liệu không được centered quanh 0, gradient descent trở nên kém hiệu quả
		\item Đây là lý do các kỹ thuật như normalization trở nên quan trọng
	\end{itemize}
	
	\item[Tanh (Hyperbolic Tangent):] Hàm tanh ánh xạ giá trị thực vào khoảng $(-1, 1)$:
	\begin{equation}
		\tanh(x) = \frac{e^x - e^{-x}}{e^x + e^{-x}} = 2\sigma(2x) - 1
	\end{equation}
	
	\paragraph{Ưu điểm so với Sigmoid}
	...
	Điều này mang lại một lợi ích quan trọng:
	
	\begin{itemize}
		\item Output được zero-centered
		\item Gradient có thể mang cả dấu dương và âm
	\end{itemize}
	
	Hệ quả:
	\begin{itemize}
		\item Các cập nhật tham số cân bằng hơn
		\item Giảm hiện tượng zig-zag
		\item Hội tụ nhanh hơn sigmoid
	\end{itemize}
	
	\paragraph{Hạn chế}
	
	Mặc dù cải thiện về mặt tối ưu hóa, tanh vẫn gặp vấn đề cốt lõi:
	
	\begin{itemize}
		\item Vẫn bị vanishing gradient khi $|x|$ lớn
		\item Saturation ở hai đầu làm gradient tiến về 0
	\end{itemize}
	
	\begin{mdframed}[style=infobox]
		\textbf{Insight}
		
		Tanh giải quyết vấn đề \textit{optimization dynamics} (zero-centered), nhưng không giải quyết vấn đề \textit{gradient flow} (vanishing gradient).
	\end{mdframed}
\end{description}

\paragraph{Kết luận}

Sigmoid và tanh minh họa hai vấn đề nền tảng trong mạng sâu:

\begin{itemize}
	\item \textbf{Gradient flow}: gradient có thể biến mất qua nhiều lớp
	\item \textbf{Optimization geometry}: phân bố của activation ảnh hưởng trực tiếp đến hướng cập nhật tham số
\end{itemize}

Sự kết hợp của hai vấn đề này là lý do chính khiến các hàm kích hoạt hiện đại như ReLU trở nên phổ biến.

\begin{center}
	\includegraphics[width=0.9\textwidth]{images/sigmoid_tanh_derivatives.png}
	\captionof{figure}{Hàm sigmoid và tanh kèm đạo hàm của chúng. Có thể thấy rõ vùng saturation nơi gradient gần như bằng 0, gây ra hiện tượng suy giảm gradient.}
...
Sự xuất hiện của hàm ReLU đánh dấu một bước ngoặt trong lịch sử deep learning. Trước đó, việc huấn luyện các mạng sâu gặp rất nhiều khó khăn do hiện tượng suy giảm gradient và động học tối ưu kém.

ReLU, với một định nghĩa cực kỳ đơn giản:

\begin{equation}
	\text{ReLU}(x) = \max(0, x)
\end{equation}

đã thay đổi hoàn toàn cách các mạng sâu được huấn luyện.

\subsubsection*{Vì sao ReLU hoạt động?}

Để hiểu tại sao ReLU hiệu quả, ta cần phân tích nó từ góc nhìn của \textbf{gradient flow} và \textbf{optimization geometry}.

\paragraph{(1) Gradient Flow: Tránh Vanishing Gradient}
...
\begin{itemize}
	\item Sigmoid: đạo hàm $\le 0.25$ và nhanh chóng tiến về 0
	\item ReLU: đạo hàm = 1 trên toàn bộ miền dương
\end{itemize}

\begin{mdframed}[style=infobox]
	\textbf{Insight cốt lõi}
	
	ReLU tạo ra một "đường dẫn gradient" (gradient highway): khi tín hiệu đi qua các neuron đang hoạt động ($x>0$), gradient không bị co lại.
\end{mdframed}

Hệ quả:

\begin{itemize}
	\item Gradient có thể lan truyền qua nhiều lớp
	\item Mạng sâu (10, 50, 100 lớp) trở nên khả thi
\end{itemize}

\paragraph{(2) Không còn Saturation ở miền dương}

Sigmoid và tanh bị \textit{saturation}:
\begin{itemize}
	\item Khi $|x|$ lớn → gradient $\approx 0$
\end{itemize}

ReLU:
\begin{itemize}
	\item Không bão hòa ở phía dương
	\item Gradient giữ nguyên = 1
\end{itemize}

Điều này đặc biệt quan trọng trong giai đoạn đầu huấn luyện, khi trọng số còn chưa ổn định.

\paragraph{(3) Cải thiện Optimization Geometry}

Một vấn đề lớn của sigmoid là output luôn dương → gây lệch gradient.

ReLU có đặc điểm:

\begin{itemize}
	\item Output có thể bằng 0 hoặc dương
	\item Phân bố activation không bị nén vào một khoảng hẹp như sigmoid
\end{itemize}

\begin{mdframed}[style=infobox]
	\textbf{Insight}
	
	ReLU không hoàn toàn zero-centered, nhưng nó giảm đáng kể bias trong gradient so với sigmoid, giúp quá trình tối ưu ổn định hơn và ít zig-zag hơn.
\end{mdframed}

\paragraph{(4) Sparsity — nhưng không chỉ là “ít neuron hoạt động”}

ReLU đặt tất cả giá trị âm về 0:

\begin{itemize}
	\item Nhiều neuron không kích hoạt tại một thời điểm
\end{itemize}

Nhưng điều quan trọng hơn:

\begin{itemize}
	\item Mỗi input kích hoạt một \textit{tập con khác nhau} của network
\end{itemize}

\begin{mdframed}[style=infobox]
	\textbf{Diễn giải sâu}
	
	Mạng ReLU có thể được xem như một tập hợp lớn các mạng tuyến tính con (piecewise linear models), được chọn động tùy theo input.
\end{mdframed}

Đây là một trong những lý do khiến ReLU có khả năng biểu diễn mạnh.

\subsubsection*{Nhược điểm: Dying ReLU}

ReLU không hoàn hảo. Vấn đề lớn nhất là hiện tượng \textbf{Dying ReLU}.

\paragraph{Cơ chế xảy ra}

Nếu một neuron rơi vào trạng thái:

\[
x = w^T \mathbf{z} + b < 0 \quad \forall \mathbf{z}
\]
...
\begin{itemize}
	\item Output luôn = 0
	\item Gradient luôn = 0
\end{itemize}

Do đó:

\begin{itemize}
	\item $\frac{\partial L}{\partial w} = 0$
	\item $\frac{\partial L}{\partial b} = 0$
\end{itemize}

Neuron này không bao giờ được cập nhật nữa.

\begin{mdframed}[style=infobox]
	\textbf{Insight quan trọng}
	
	Dying ReLU không chỉ là "output = 0", mà là \textit{mất hoàn toàn gradient}, khiến neuron bị loại vĩnh viễn khỏi quá trình học.
\end{mdframed}

\paragraph{Nguyên nhân}

\begin{itemize}
	\item Learning rate quá lớn → cập nhật nhảy quá xa
	\item Khởi tạo trọng số không tốt
	\item Bias âm lớn
\end{itemize}
...
\begin{itemize}
	\item Một phần mạng bị "tắt"
	\item Giảm năng lực biểu diễn
\end{itemize}

\subsubsection*{Nhìn từ góc độ tổng thể}

ReLU không phải là hàm "hoàn hảo", nhưng nó đạt được một cân bằng cực kỳ quan trọng:

\begin{itemize}
	\item Giữ gradient đủ lớn để học sâu
	\item Tránh saturation ở vùng quan trọng
	\item Đơn giản về tính toán
\end{itemize}

\begin{mdframed}[style=infobox]
	\textbf{Insight tổng kết}
	
	ReLU thành công không phải vì nó "đúng về mặt sinh học", mà vì nó phù hợp với động học tối ưu của gradient descent trong không gian nhiều chiều.
\end{mdframed}
...
\label{subsec:weight_initialization}

Cho đến thời điểm này, chúng ta đã thảo luận về các thành phần của một mạng nơ-ron: phép tích chập, hàm kích hoạt, và pooling. Tuy nhiên, còn một yếu tố quan trọng thường bị xem nhẹ nhưng lại có ảnh hưởng quyết định đến việc mạng có học được hay không:

\begin{center}
	\textit{Mạng nơ-ron bắt đầu học từ đâu?}
\end{center}

Câu trả lời nằm ở \textbf{khởi tạo trọng số}.

\subsubsection*{Vấn đề: Lan truyền tín hiệu qua nhiều lớp}

Xét một mạng nhiều lớp. Tại mỗi lớp:

\[
\mathbf{h}^{(l)} = W^{(l)} \mathbf{h}^{(l-1)}
\]

Nếu các phần tử của $W^{(l)}$ được khởi tạo ngẫu nhiên, thì sau nhiều lớp:

\begin{itemize}
	\item Nếu giá trị quá lớn → tín hiệu phình ra (exploding)
	\item Nếu giá trị quá nhỏ → tín hiệu suy giảm (vanishing)
\end{itemize}

\begin{mdframed}[style=infobox]
	\textbf{Insight cốt lõi}
	
	Mục tiêu của weight initialization là giữ cho phương sai (variance) của tín hiệu được \textbf{bảo toàn} khi lan truyền qua các lớp.
\end{mdframed}

\subsubsection*{Phân tích phương sai}

Giả sử:
\begin{itemize}
	\item $x$ có mean = 0, variance = $\sigma^2$
	\item $w$ được khởi tạo độc lập
\end{itemize}

Khi đó:

\[
\text{Var}(Wx) \approx n \cdot \text{Var}(w) \cdot \text{Var}(x)
\]

trong đó $n$ là số lượng input (fan-in).

Để giữ variance không đổi:
...
Được đề xuất cho các hàm kích hoạt như sigmoid hoặc tanh.
...
\begin{itemize}
	\item Cân bằng giữa forward pass và backward pass
	\item Tránh việc tín hiệu bị phóng đại hoặc suy giảm
\end{itemize}

\begin{mdframed}[style=infobox]
	\textbf{Insight}
	
	Xavier initialization giả định rằng activation là đối xứng quanh 0 (như tanh), do đó nó không tối ưu cho ReLU.
\end{mdframed}

---

\subsubsection{He Initialization (Kaiming Initialization)}

Được thiết kế đặc biệt cho ReLU.
...
\paragraph{Tại sao cần hệ số 2?}

ReLU "tắt" một nửa tín hiệu (vì $x<0 \rightarrow 0$), nên:

\begin{itemize}
	\item Chỉ khoảng 50\% neuron truyền tín hiệu
	\item Phương sai bị giảm một nửa
\end{itemize}

Do đó cần nhân hệ số 2 để bù lại.

\begin{mdframed}[style=infobox]
	\textbf{Insight cực quan trọng}
	
	He initialization không phải là một công thức ngẫu nhiên — nó trực tiếp xuất phát từ việc ReLU loại bỏ một nửa phân phối.
\end{mdframed}

---

\subsubsection{Hậu quả của Khởi tạo Sai}

Nếu khởi tạo không phù hợp:

\begin{itemize}
	\item Gradient biến mất ngay từ đầu
	\item Hoặc bùng nổ → training không ổn định
	\item Mạng hội tụ rất chậm hoặc không hội tụ
\end{itemize}

\begin{center}
	\includegraphics[width=0.85\textwidth]{images/weight_initialization_effect.png}
	\captionof{figure}{Ảnh hưởng của khởi tạo trọng số. Nếu variance quá nhỏ hoặc quá lớn, tín hiệu sẽ bị suy giảm hoặc bùng nổ khi đi qua nhiều lớp. Khởi tạo đúng giúp giữ ổn định phân phối kích hoạt.}
	\label{fig:weight_initialization}
\end{center}

---

\subsubsection{Mối liên hệ với Activation}

\begin{center}
	\begin{tabular}{|c|c|}
		\hline
		\textbf{Activation} & \textbf{Initialization phù hợp} \\
		\hline
		Sigmoid / Tanh & Xavier \\
		ReLU / Leaky ReLU & He \\
		GELU / Swish & He (thường dùng) \\
		\hline
	\end{tabular}
\end{center}
...
Weight initialization là một trong những yếu tố quan trọng nhất nhưng ít được chú ý trong thiết kế mạng nơ-ron.

\begin{mdframed}[style=infobox]
	\textbf{Insight tổng kết}
	
	Activation function quyết định \textit{dạng biến đổi phi tuyến}, còn weight initialization quyết định \textit{liệu tín hiệu và gradient có thể tồn tại đủ lâu để học hay không}.
\end{mdframed}
